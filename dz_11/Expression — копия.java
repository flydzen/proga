...
public class Expression implements Parser {
    ...
    public static class ExpressionParser extends BaseParser {
        ...
        public ExpressionParser(final ExpressionSource source) {
            ...
            mapBinOps.put('p', new Pair(Opers.POW2, 4));
            mapBinOps.put('^', new Pair(Opers.POW, 5));
            mapBinOps.put('_', new Pair(Opers.LOG, 5));

            isUno.add(Opers.UNO);
            isUno.add(Opers.ABS);
            isUno.add(Opers.SQRT);
            isUno.add(Opers.LOG2);
            isUno.add(Opers.POW2);
            nextChar();
        }
        ...
        private void parseElement() {
            ...
            if (test('(')) {
                ...
                if (!(isUno.contains(opers.peek().op) && isUno.contains(cur.op))) {
                    while (true) {
                        ...
                        if (opers.peek().op == Opers.UNO) {
                            ...
                            old = mapBinOps.get(opers.peek().ch).prior;
                        }
                ...
                if (cur.op == Opers.LOG)
                    opers.add(new Line(cur.op, time++, '_'));
                else if (cur.op == Opers.POW)
        ...
        private void compare() {
            ...
            if (isUno.contains(action)) {
                ...
                } else if (action == Opers.LOG2) {
                    values.add(new Line(new Log2(e), time++));
                } else if (action == Opers.POW2) {
                    values.add(new Line(new Pow2(e), time++));
                }
            } else {
                if (values.size() == 1) {
                ...
                } else if (action == Opers.POW) {
                    values.add(new Line(new Pow(e1, e2), time++));
                } else if (action == Opers.LOG) {
                    values.add(new Line(new Log(e1, e2), time++));
                }
            }
        }
