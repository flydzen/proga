 ...
 */
public class WordStatSortedFirstIndexTest {
    private final WordStatIndexChecker checker;

    public WordStatSortedFirstIndexTest(final String className) {
        checker = new WordStatIndexChecker(className);
    }

    public static void main(final String... args) {
        new WordStatSortedFirstIndexTest("WordStatSortedFirstIndex").run();
    }
    ...
    protected List<Pair<String, String>> answer(final String[][] text) {
        final Map<String, Integer> totals = Arrays.stream(text)
                ...
                .collect(Collectors.toMap(Function.identity(), k -> 1, Integer::sum, TreeMap::new));
        ...
        Arrays.stream(text)
                .map(line -> IntStream.range(0, line.length).boxed()
                        .collect(Collectors.toMap(i -> line[i].toLowerCase(), i -> i + 1, Math::min, HashMap::new)))
