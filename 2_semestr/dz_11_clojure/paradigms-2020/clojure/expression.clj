...
(deftype CVProto_ [ev, st, difAr, value]
    Expression
    (evaluate [this vars] (ev value vars))
    (string [this] (st value))
    (diff [this diffed] diffed)
    (diffArgs [this var] (difAr value var))
...
(defn CVCreator [ev st difAr] (fn [value] (CVProto_. ev st difAr value)))
(defn binCreator [op ch fdiff] (fn [& f] (binOperProto. op ch fdiff f)))
(defn unoCreator [op ch fdiff] (fn [f] (unOperProto. op ch fdiff f)))

(def Constant (CVCreator
    (fn [value vars] value)
    (fn [value] (format "%.1f" value))
    (fn [value var] Zero)))
(def Variable (CVCreator
    (fn [value vars] (get vars (str value)))
    (fn [value] (str value))
    (fn [value var] (if (= value var) One Zero ))))
(def Zero (Constant 0))
(def One (Constant 1))
